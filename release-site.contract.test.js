...
const versions = ['3.4.8', '3.4.9', '3.4.10', '3.4.11', '3.4.12'];
const baselineCounts = {'3.4.8': 12, '3.4.9': 17, '3.4.10': 24, '3.4.11': 38, '3.4.12': 0};
const releaseStarts = {'3.4.8': 10, '3.4.9': 10, '3.4.10': 11, '3.4.11': 12, '3.4.12': 0};
...
assert.match(index, /<span class="keep-together">浙江区域扫描<\/span>/);
assert.match(index, /<span class="keep-together">浙江军团<\/span>/);
assert.match(index, /<span class="keep-together">代码检测<\/span>仅扫描<span class="keep-together">浙江军团<\/span>、<span class="keep-together">浙江大区<\/span>、<span class="keep-together">江浙大区<\/span>/);
assert.match(index, /<span class="keep-together">区域精确过滤<\/span>/);
assert.match(index, /<span class="keep-together">上海军团<\/span>/);
assert.match(index, /明确排除<span class="keep-together">上海军团<\/span>及<span class="keep-together">其他区域项目<\/span>/);
assert.match(index, /<span class="keep-together">延期状态高亮<\/span>/);
assert.match(index, /红色加粗显示/);
assert.match(index, /<span class="delay-emphasis keep-together">“延期”时<\/span>红色加粗显示/);
